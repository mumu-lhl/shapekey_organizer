...
_zh_fallback = {
    ...
    "Restore Default Shape Key Order": "恢复默认形态键排序",
    "Restore category shape key order to the current mesh's original shape key order": "将分类内形态键排序恢复为当前网格的原始形态键顺序",
    "Restored default shape key order for current mesh.": "已恢复当前网格的默认形态键排序。",
    ...
    "Alias": "别名",
    "Alias Editor": "别名编辑器",
    "AI Alias Helper": "AI 别名助手",
    "AI Alias Prompt": "AI 别名 Prompt",
    "Use Default AI Alias Prompt": "使用默认 AI 别名 Prompt",
    "Restore the default prompt template for AI alias generation": "恢复 AI 别名生成的默认 Prompt 模板",
    "Restored default AI alias prompt": "已恢复默认 AI 别名 Prompt",
    "Copy AI Alias Prompt": "复制 AI 别名 Prompt",
    "Copy Prompt": "复制 Prompt",
    "Copy a prompt for generating readable aliases for shape keys without aliases": "复制一个用于为未设置别名的形态键生成清晰易读别名的 Prompt",
    "AI Alias JSON": "AI 别名 JSON",
    "Paste AI-returned JSON here, then apply aliases.": "在这里粘贴 AI 返回的 JSON，然后应用别名。",
    "For multiline JSON, copy it and use Paste JSON from Clipboard.": "多行 JSON 请先复制，再使用从剪贴板粘贴 JSON。",
    "Paste AI Alias JSON from Clipboard": "从剪贴板粘贴 AI 别名 JSON",
    "Paste multiline AI alias JSON from the system clipboard into the alias JSON field": "从系统剪贴板将多行 AI 别名 JSON 粘贴到 JSON 输入框",
    "Clipboard is empty": "剪贴板为空",
    "Pasted AI alias JSON from clipboard": "已从剪贴板粘贴 AI 别名 JSON",
    "Paste the JSON object returned by the AI, using exact shape key names as keys and aliases as values": "粘贴 AI 返回的 JSON 对象，键为精确形态键名称，值为别名",
    "Apply AI Alias JSON": "应用 AI 别名 JSON",
    "Set aliases from a JSON object whose keys are exact shape key names and values are aliases": "从 JSON 对象设置别名，其中键是精确形态键名称，值是别名",
    "Paste AI alias JSON first": "请先粘贴 AI 别名 JSON",
    "Invalid AI alias JSON: {}": "无效的 AI 别名 JSON: {}",
    "AI alias JSON must be an object": "AI 别名 JSON 必须是对象",
    "Applied {} AI alias(es). Skipped {} item(s).": "已应用 {} 个 AI 别名，跳过 {} 项。",
    "No matching AI aliases were applied": "没有应用任何匹配的 AI 别名",
    "Target Language": "目标语言",
    "Target language for AI-generated shape key aliases": "AI 生成形态键别名时使用的目标语言",
    "Prompt template used when copying AI alias generation prompts. Supports {target_language} and {shape_key_names}": "复制 AI 别名生成 Prompt 时使用的模板，支持 {target_language} 和 {shape_key_names}",
    "All shape keys already have aliases": "所有形态键都已经设置了别名",
    "Copied AI alias prompt for {} shape key(s)": "已复制包含 {} 个形态键的 AI 别名 Prompt",
    "Preview Value": "预览值",
    ...
    "Sorted shape keys within categories using {} non-zero current-project frequencies": "已使用 {} 个非零当前工程频次对各分类内部的形态键排序",
    ...
    "Sorted shape keys within categories using {} saved frequencies": "已使用 {} 个保存的频次对各分类内部的形态键排序",
